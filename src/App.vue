...
export default {
  ...
  methods: {
    addFn() {
      if (!this.price || !this.name) return alert('请输入完整')
      this.list.push({
  ...
  computed: {
    allPrice() {
      return this.list.reduce((pre, item) => pre + item.price, 0).toFixed(2)
    },
    avPrice() {
  ...
  watch: {
    list: {
      deep: true,
      handler(val) {
        return localStorage.setItem('list', JSON.stringify(val))
      },
    },
  },
}
